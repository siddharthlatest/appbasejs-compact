  ...
  var internalFunctions = {
    ...
    onEdges: function(event, filters, interfaceCallback) {
      var reqData = {
        ...
        var filterString = ab.util.generateFilterString(reqData)
        if(ab.server.edges.urlsListening[exports.URL() + filterString]) {
          if(event == "edge_added")
            setTimeout(function() {
              ab.firing.prepareForEdges('RETR', exports.URL() + filterString, {},
                ab.cache.get('edges', exports.URL() + filterString), interfaceCallback)
            },0)
        }
        amplify.subscribe(event+":"+exports.URL() + filterString, referenceID, interfaceCallback)
        if(!ab.server.edges.urlsListening[exports.URL() + filterString]) {
          ab.server.edges.listen(exports.URL(), reqData, function(error, request) {
            if(error) {
              interfaceCallback(error)
              amplify.unsubscribe(event+":"+exports.URL() + filterString, referenceID)
            }
          })
        }
      }
