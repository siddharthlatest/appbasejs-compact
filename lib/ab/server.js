...
ab.server.edges = {
  ...
  listen: function(url, requestdata, callback) {
    // url format - 'http://appname.localhost:5005/Materials/Ice/dad/dsd'
    var data = ab.util.parseURL(url)
    var filterString = ab.util.generateFilterString(requestdata)
    data["filters"] = requestdata.filters
    data["listener_id"] = ab.util.uuid()
    ab.util.setCredsInData(data)
    var event = JSON.stringify(data)
    ...
    ab.server.realtime.socket.on(event, listener = function(result) {
      if(typeof result === 'string') {
        delete ab.server.edges.urlsListening[url + filterString]
        if(result === 'STOPPED') {
          ab.server.realtime.socket.removeListener(event,listener)
          return
        }
        try {
          ...
          console.error(url + filterString,e)
        }
      } else {
        var previous = ab.cache.get("edges", url + filterString)
        try {
          switch(result.optype) {
            case 'REMOVE ALL':
              ab.cache.remove("edges", url + filterString)
              ab.firing.prepareForEdges(result.optype, url + filterString, previous, {})
              break
            case 'REMOVE':
              ab.cache.remove("edges", url + filterString, Object.keys(result.edges))
              ab.firing.prepareForEdges(result.optype, url + filterString, previous, result.edges)
              break
            default:
              ...
              ab.cache.set("edges", url + filterString, result.edges)
              ab.firing.prepareForEdges(result.optype, url + filterString, previous, result.edges)
          }
        } catch(e) {
          console.error(url + filterString,e)
        }
        result.edgeCache = ab.cache.get("edges", url + filterString)
        try {
          callback(null, result)
        } catch(e) {
          console.error(url + filterString,e)
    ...
    ab.server.edges.urlsListening[url + filterString] = data
