...
ab.server.realtime.socket.on('reconnect', function() {
  ...
  ab.server.realtime.emittedEvents.forEach(function(event) {
    event.data.timestamp = ab.cache.timestamps[event.url]
      [event.type === 'properties'?'vertex':event.type]
      ...
      if(typeof result === 'string') {
        ...
        switch(result.optype) {
          ...
          default :
            ...
            ab.firing.prepareForEdges(result.optype, url, previous, sresult.edges)
        }
...
ab.firing = {
  ...
  prepareForEdges: function(optype, url, previous, current, callback) {
    switch(optype) {
      ...
      default :
        for(var edgeName in current) {
          if(previous[edgeName] !== undefined) ab.firing.fire(url, "edge_changed", 
...
Appbase.credentials = function(appName, appSecret) {
  ...
  ab.server.setAppSecret(appSecret)
}
