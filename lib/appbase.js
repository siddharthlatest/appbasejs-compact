...
var socket = io('http://app.sagar.appbase.io:80/')
    ...
    socket.on(JSON.stringify(data), function(result) {
      if(typeof result === 'string') {
        ...
        switch(result.optype) {
          case 'REMOVE ALL':
            ab.cache.remove("vertex", url, undefined)
            break;
          case 'REMOVE':
  ...
  listen: function(url, requestdata, callback) {
    ...
    socket.on(JSON.stringify(data), function(result) {
      if(typeof result === 'string') {
        ...
        switch(result.optype){
          case 'REMOVE ALL':
            ab.cache.set("vertex", url, undefined)
            break;
          case 'REMOVE':
            ab.cache.remove("vertex",url,result.vertex)
            break;
          default :
            ab.cache.set("vertex",url,result.vertex)
        }
...
ab.interface.ref = function(path) {
  ...
  exports.setEdge = function(edgeData, callback) {
    ...
    var data = {}
    var parsedURL = ab.util.parseURL(edgeData.ref.path())
  ...
  exports.removeEdge = function(edgeData,callback) {
    ab.server.edges.delete(path,[edgeData.name],function(error,result) {
      if(!error) {
        callback(error,exports)
      } else {
        callback(error)
