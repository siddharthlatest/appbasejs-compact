...
ab.firing = {
  prepareForProperties: function(optype,url,previous,current) {
    ab.firing.fire(url,"properties",previous,current)
  },
  prepareForEdges: function(optype,url,previous,current) {
    switch(optype){
      case "REMOVE ALL":
        for(var edgeName in previous) {
          ab.firing.fire(url, "edge_remove", previous[edgeName], {}, edgeName)
        }
        break
      case "REMOVE":
      ...
      default :
        for(var edgeName in current) {
          if(previous[edgeName] !== undefined) ab.firing.fire(url, "edge_changed", previous[edgeName], current[edgeName], edgeName)
          else ab.firing.fire(url, "edge_added", {},current[edgeName], edgeName)
        }
  ...
  ,
  fire: function(url, event, previous, current, edgeName) {
    switch(event) {
      case 'edge_added':
      case 'edge_changed':
      case 'edge_removed':
        var edgeSnapshot = ab.util.createEdgeSnapshot(previous,current,edgeName)
        var outVertexSnapshot = ab.util.createVertexSnapshot({},ab.cache.get("vertex",url+'/'+edgeName)) //TODO: doesn't work
        console.log(event,url,edgeSnapshot.name(),edgeSnapshot.prevPriority(),edgeSnapshot.priority(),outVertexSnapshot.properties())
        break
      case 'properties':
        var vertexSnapshot = ab.util.createVertexSnapshot(previous,current)
        console.log(event,url,vertexSnapshot.properties(),vertexSnapshot.prevProperties())
    }
  }
}
