 ...
 * @api public
  ...
  (function(){
    ...
    ab.server.realtime.socket = io('http://api1.appbase.io:80/');
    ...
    ab.server.timestamp = function(callback) {
    ...
    ab.cache.ns = {
      memStore: {},
      timestamps: {},
      set: function(url, pKey, data) {
        console.log('SET', url, pKey, data)
        if (!ab.cache.ns.memStore.hasOwnProperty(url)) {
          ab.cache.ns.memStore[url] = {}
        }
        if(!ab.cache.ns.memStore[url].hasOwnProperty(pKey)) {
          ab.cache.ns.memStore[url][pKey] = {}
        }

        for (var key in data) {
          if((key !== '_id' && key !== 'timestamp' && key !== 'rootPath')) {
            ab.cache.ns.memStore[url][pKey][key] = data[key]
          }
        }

        ab.cache.ns.timestamps[url] = (ab.cache.ns.timestamps[url] === undefined || ab.cache.ns.timestamps[url] < data['timestamp']) ?
          data['timestamp'] : ab.cache.ns.timestamps[url]
      },
      get: function(url, pKey, isClone) {
        console.log('GET', url, pKey)
        var isClone = (isClone === undefined ? true : isClone)
        if (isClone && ab.cache.ns.memStore.hasOwnProperty(url) && ab.cache.ns.memStore.hasOwnProperty(url)[pKey]) {
          return JSON.parse(JSON.stringify(ab.cache.ns.memStore[url][pKey]))
        } else if (!isClone && (ab.cache.ns.memStore.hasOwnProperty(url)  && ab.cache.ns.memStore.hasOwnProperty(url)[pKey])) {
          return ab.cache.ns.memStore[url][pKey]
        } else {
          return {}
        }
      },
      remove: function(url, pKey, data) {
        console.log('REMOVE', url, pKey, data)
        if (ab.cache.ns.memStore.hasOwnProperty(url) && typeof data === "undefined")
          ab.cache.ns.memStore[url] = {}
        else if (ab.cache.ns.memStore.hasOwnProperty(url)) {
          for (var key in data)
            delete ab.cache.ns.memStore[url][pKey][data[key]]
        }
      }
    }

    ab.server.search = function(namespace, query, callback) {
      atomic.post(ab.server.baseURL + '/' + namespace + "/~search", {"query": query, "secret": ab.server.getAppSecret()})
    ...
    ab.server.ns = {
      ...
      listen: function(url, requestdata, callback) {
        ...
        ab.server.realtime.socket.on(event, listener = function(result) {
          if(typeof result === 'string') {
            ...
            try {
              if(result[0] !== undefined) { //array
                result.forEach(function(v) {
                  var pKey = v.rootPath.slice(v.rootPath.indexOf('/') + 1)
                  var previous = ab.cache.ns.get(url, pKey)
                  ab.cache.ns.set(url, pKey, v)
                  //fire
                })
              } else {
                var v = result.vertex
                ...
                switch(result.optype) {
                  case 'REMOVE ALL':
                    ab.cache.ns.remove(url, pKey)
                    break
                  case 'REMOVE':
                    ab.cache.ns.remove(url, pKey, Object.keys(v))
                    break
                  default :
                    ab.cache.ns.set(url, pKey, v)
                }
      ...
      unlisten: function(url) {
        ...
        //empty cache
        ab.cache.ns.memStore[url] && (ab.cache.ns.memStore[url] = {})
        if(data) {
    ...
    ab.server.vertex = {
      ...
      get: function(url, callback){
        atomic.post(url + "/~properties", {"data":[""], "secret": ab.server.getAppSecret()})
          .success(function(result) {
            if(typeof result === 'string') {
              callback(new Error(result))
            } else {
    ...
    ab.interface.vertex = function(path) {
      ...
      exports.setEdge = function(name, ref, priority, callback) {
        if(arguments.length === 2 && typeof priority === 'function') {
          var callback = ref
          ref = undefined
        } else if(arguments.length === 3 && typeof priority === 'function') {
        ...
        if(ref && ref.isV) {
          throw "Invalid Appbase Reference for setEdge."
        }
    ...
    ab.server.timestamp = function(cb) {
      atomic.get("http://api1.appbase.io/time")
        .success(function(result) {
          cb(null, result)
        })
