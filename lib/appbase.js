...
ab.server.realtime.emit = function(args){
  ab.server.realtime.emittedEvents.push(args)
  ab.server.realtime.socket.emit.apply(ab.server.realtime.socket,args)
}
...
ab.server.vertex = {
  ...
  listen: function(url, requestdata, callback) {
    ...
    ab.server.realtime.emit(["properties", JSON.stringify(data)])
  },
...
ab.server.edges = {
  ...
  listen: function(url, requestdata, callback) {
    ...
    ab.server.realtime.emit(["edges", JSON.stringify(data)])
  },
  delete: function(url, data, callback) {
...
Appbase.create = function(namespace,key, callback) {
  if(typeof key === "function" && callback === undefined){
    var callback = key
    key = ab.util.uuid()
  } else if (namespace && key && callback){
  } else {
    throw 'Invalid arguments'
  }
