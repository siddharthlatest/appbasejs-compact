 ...
 * @api public
  ...
  (function(){
    ...
    ab.server.realtime.socket = io('http://api2.appbase.io:80/');
    ...
    ab.server.vertex = {
      ...
      destroy: function(url, callback) {
        atomic.delete(url, {all: true, secret: ab.server.getAppSecret()})
          .success(function(result) {
            if(typeof result === 'string') {
              callback(new Error(result))
            } else {
    ...
    ab.auth = {
      config: {
        oauthdURL: 'http://localhost:6284/',
        oauthMiddleSever: 'http://localhost:3000',
        oauthTokenURL: '/oauth/signin',
        oauthStateURL: '/oauth/token'
      },
      getStateToken: function(cb) {
        atomic.get(ab.auth.config.oauthMiddleSever + ab.auth.config.oauthStateURL)
    ...
    ab.interface.auth = {
      popup: function(provider, options, cb) {
        var tO;
        if( (tO = typeof options) === 'object') {
          if(tO === 'function') {
            var cb = options
          }
          var options = {}
        }
        ab.auth.getStateToken(function(error, state) {
          if(error) {
            return cb(error)
          }
          options.state = state
          OAuth.popup(provider, options)
            .done(function (result) {
              ab.auth.codeToCreds(provider, result.code, function(error, creds) {
                if(error) {
                  return cb(error)
                }
                var requestObj = OAuth.create(provider, creds.credentials.provider)
                cb(null, creds, requestObj)
              })
            })
            .fail(function (error) {
              cb(error)
    ...
    Appbase.credentials = function(appName, appSecret) {
      ...
      ab.auth.initOauthio(appName)
      ab.server.setApp(appName)
      return true
    }

    ab.server.timestamp = function(cb) {
      atomic.get("http://api2.appbase.io/time")
        .success(function(result) {
          cb(null, result)
        })
