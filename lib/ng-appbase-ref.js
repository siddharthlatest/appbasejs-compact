...
angular.module('ngAppbase',[])
  .factory('$appbaseRef',function() {
    var ngAppbaseRef = function(input) {
      ...
      var edgeRefs = {}
      var bindEdges = function(remoteScope, varName, bindOutVertex, bindSecondLevelEdges, onAdded) {
        ...
        var add = function(edgeSnap, edgeRef) {
          ...
          edgeRef.on('properties',function(error, r, outVertexSnap) {
            ...
            var proceed = function() {
              ...
              if(!added) {
                onAdded && onAdded(edgeObj, edgeRef, remoteScope.$apply.bind(remoteScope, addEdgeToArray))
                !onAdded && addEdgeToArray()
              }
            }
            !onAdded && remoteScope.$apply(proceed)
            onAdded && proceed()
          })
