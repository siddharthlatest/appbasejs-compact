...
angular.module('ngAppbase',[])
  .factory('$appbaseRef',function() {
    var ngAppbaseRef = function(input) {
      ...
      var edgeRefs = {}
      var callbacks
      var bindEdges = function(remoteScope, varName, bindOutVertex, bindSecondLevelEdges, cb) {
        if(typeof varName === "boolean") {
          callbacks = bindSecondLevelEdges || {}
          var bindSecondLevelEdges = bindOutVertex
          var bindOutVertex = varName
          varName = undefined
        } else {
          callbacks = cb || {}
        }
        var secondLevelBinding = bindOutVertex || bindSecondLevelEdges
        ...
        var add = function(edgeSnap, edgeRef) {
          ...
          edgeRef.on('properties',function(error, r, outVertexSnap) {
            ...
            var proceed = function() {
              ...
              if(!added) {
                callbacks.onAdd && callbacks.onAdd(edgeObj, edgeRef, remoteScope.$apply.bind(remoteScope, addEdgeToArray))
                !callbacks.onAdd && addEdgeToArray()
              }
            }
            !callbacks.onAdd && remoteScope.$apply(proceed)
            callbacks.onAdd && proceed()
          })
      ...
      var unbindEdges = function(){
        ...
        for(var edgeName in edgeRefs) {
          edgeRefs[edgeName].off()
        }
      }
